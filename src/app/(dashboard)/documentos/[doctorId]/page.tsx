...
export default function DoctorDocumentsPage({ params }: { params: Promise<{ doctorId: string }> }) {
  ...
  const doctorShifts = store.shifts.filter(shift => shift.organizationId === store.activeOrganizationId && shift.doctorId === doctorId);
  const operationalUnitIds = getDoctorOperationalUnitIds(doctorShifts, doctorId);
  const operationalDoctor = doctor ? { ...doctor, linkedUnits: operationalUnitIds } : null;
  const doctorDocuments = operationalDoctor ? applicableDocuments(operationalDoctor, organizationDocuments, requirements).sort((a, b) => documentStatusOrder[a.status] - documentStatusOrder[b.status] || a.name.localeCompare(b.name, 'pt-BR')) : [];
  const linkedUnits = store.units.filter(unit => unit.organizationId === store.activeOrganizationId && operationalUnitIds.includes(unit.id));
  const summary = operationalDoctor ? getDoctorCompliance(operationalDoctor, organizationDocuments, referenceTime, requirements, Math.max(...governance.expiryAlertDays)) : null;
    ...
    <AccessGuard requiredPermission="documentos">
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <section className="grid overflow-hidden rounded-2xl border border-border bg-card-bg sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_1.4fr]">
          ...
          <div className="border-t border-border p-5 sm:border-l xl:border-t-0"><p className="text-xs text-text-muted">Unidades atendidas</p><div className="mt-2 flex flex-wrap gap-1.5">{linkedUnits.map(unit => <span key={unit.id} className="inline-flex items-center gap-1 rounded-md bg-surface-muted px-2 py-1 text-[11px] font-medium text-text-secondary"><Building2 size={12}/>{unit.name}</span>)}{!linkedUnits.length && <span className="text-xs text-text-muted">Sem plantões cadastrados</span>}</div></div>
        </section>
