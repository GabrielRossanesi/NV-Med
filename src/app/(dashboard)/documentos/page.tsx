...
import { documentIsCritical, documentIsNearExpiry, documentNeedsAction, documentStatusClasses, documentStatusLabels, documentStatusOrder, getDoctorCompliance } from '@/lib/documentCompliance';
import { getDocumentGovernance, requirementApplies } from '@/lib/documentGovernance';
import { doctorContractModelShortLabels } from '@/lib/doctorProfile';
...
function DocumentCenter() {
  ...
  const { activeOrganizationId, doctors, documents, units, shifts, organizations, currentUser } = store;
  ...
  const orgShifts = useMemo(() => shifts.filter(shift => shift.organizationId === activeOrganizationId), [shifts, activeOrganizationId]);
  const operationalDoctors = useMemo(() => new Map(orgDoctors.map(doctor => [doctor.id, { ...doctor, linkedUnits: getDoctorOperationalUnitIds(orgShifts, doctor.id) }])), [orgDoctors, orgShifts]);
  ...
  const summaries = useMemo(() => new Map(orgDoctors.map(doctor => [doctor.id, getDoctorCompliance(operationalDoctors.get(doctor.id) || doctor, orgDocuments, referenceTime, requirements, Math.max(...governance.expiryAlertDays))])), [orgDoctors, operationalDoctors, orgDocuments, referenceTime, requirements, governance.expiryAlertDays]);
  ...
  const filteredDoctors = orgDoctors
    ...
    .filter(doctor => !search || `${doctor.name} ${doctor.crm} ${doctor.rqe || ''} ${doctor.specialty}`.toLowerCase().includes(search.toLowerCase()))
    .filter(doctor => doctorHasOperationalUnit(orgShifts, doctor.id, unitId))
  ...
  const filteredDocuments = orgDocuments
    ...
    .filter(document => {
      ...
      return Boolean(doctor && requirement && requirementApplies(requirement, operationalDoctors.get(doctor.id) || doctor));
    })
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <section className="rounded-2xl border border-border bg-card-bg p-4" aria-label="Filtros de documentação">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(220px,1.4fr)_1fr_1fr_1fr_auto]">
          <label className="nv-label">Buscar médico<div className="relative mt-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted"/><input className="nv-input pl-9" value={search} onChange={event => setSearch(event.target.value)} placeholder="Nome, CRM, RQE ou especialidade"/></div></label>
