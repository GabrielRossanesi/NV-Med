...
import { ArrowLeft, Building2, CalendarDays, MapPin, Moon, Pencil, Plus, Stethoscope, Sun, UserRound, Users } from 'lucide-react';
import AccessGuard from '@/components/AccessGuard';
import Dialog from '@/components/Dialog';
import { canEditPermission } from '@/lib/permissions';
import { coveragePeriodLabel, localDate, sectorCoveragePeriods } from '@/lib/scheduling';
import { useStore } from '@/store/useStore';
import type { Sector } from '@/types';

type SectorDraft = Omit<Sector, 'id' | 'organizationId' | 'unitId'> & { id?: string };
const emptySector: SectorDraft = { name: '', specialties: [], status: 'active', defaultStartTime: '07:00', defaultEndTime: '19:00', requiredDoctors: 1, coveragePeriods: [{ kind: 'day', startTime: '07:00', endTime: '19:00', requiredDoctors: 1 }] };
...
export default function UnitDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function openSector(sector?: Sector) {
    ...
    setDraft(sector ? { id: sector.id, name: sector.name, specialties: sector.specialties, status: sector.status, defaultStartTime: sector.defaultStartTime, defaultEndTime: sector.defaultEndTime, requiredDoctors: sector.requiredDoctors, coveragePeriods: sectorCoveragePeriods(sector).map(period => ({ ...period })) } : { ...emptySector, coveragePeriods: emptySector.coveragePeriods.map(period => ({ ...period })) });
  }
  async function saveSector(event: React.FormEvent) {
    ...
    if (!draft.coveragePeriods.length) return setFormError('Selecione ao menos um período de cobertura.');
    if (draft.coveragePeriods.some(period => period.startTime === period.endTime)) return setFormError('Início e fim de cada período precisam ser diferentes.');
    const primary = draft.coveragePeriods[0];
    const payload = { ...draft, name: draft.name.trim(), specialties: [...new Set(draft.specialties.map(item => item.trim()).filter(Boolean))], defaultStartTime: primary.startTime, defaultEndTime: primary.endTime, requiredDoctors: primary.requiredDoctors };
  ...
  function toggleCoverage(kind: 'day' | 'night') {
    if (!draft) return;
    const existing = draft.coveragePeriods.find(period => period.kind === kind);
    const coveragePeriods = existing ? draft.coveragePeriods.filter(period => period.kind !== kind) : [...draft.coveragePeriods, kind === 'day' ? { kind, startTime: '07:00', endTime: '19:00', requiredDoctors: 1 } : { kind, startTime: '19:00', endTime: '07:00', requiredDoctors: 1 }];
    setDraft({ ...draft, coveragePeriods });
  }
  function updateCoverage(kind: 'day' | 'night', changes: Partial<Sector['coveragePeriods'][number]>) {
    if (!draft) return;
    setDraft({ ...draft, coveragePeriods: draft.coveragePeriods.map(period => period.kind === kind ? { ...period, ...changes } : period) });
  }
  ...
  return <AccessGuard requiredPermission="unidades"><div className="space-y-7">
    ...
    <div className="grid gap-7 xl:grid-cols-[minmax(0,1.5fr)_minmax(290px,.7fr)]">
      <section className="min-w-0">
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card-bg">
          {sectors.length === 0 ? <div className="p-10 text-center"><Stethoscope className="mx-auto mb-3 text-text-muted"/><h3 className="font-semibold">Nenhum setor cadastrado</h3><p className="mt-1 text-sm text-text-secondary">{canEditUnit ? 'Cadastre o primeiro setor para montar a grade semanal.' : 'Ainda não há setores configurados nesta unidade.'}</p></div> : <ul className="divide-y divide-border">{sectors.map(sector => <li key={sector.id} className="flex flex-wrap items-center justify-between gap-4 p-4 hover:bg-state-hover"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="font-semibold">{sector.name}</p>{sector.status === 'inactive' && <span className="rounded bg-surface-muted px-2 py-0.5 text-[11px] text-text-muted">Inativo</span>}</div><p className="mt-1 text-sm text-text-secondary">{sector.specialties.length ? sector.specialties.join(' · ') : 'Especialidade ainda não definida'}</p><div className="mt-3 flex flex-wrap gap-2">{sectorCoveragePeriods(sector).map(period => <span key={period.kind} className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface-muted/50 px-2.5 py-1.5 text-xs text-text-secondary">{period.kind === 'day' ? <Sun size={13} className="text-warning"/> : <Moon size={13} className="text-primary"/>}<strong className="text-text-primary">{coveragePeriodLabel(period.kind)}</strong><span className="tabular-nums">{period.startTime}–{period.endTime}</span><span className="inline-flex items-center gap-1"><Users size={12}/>{period.requiredDoctors}</span></span>)}</div></div>{canEditUnit && <button className="p-2.5 text-text-muted hover:text-primary" aria-label={`Editar ${sector.name}`} onClick={() => openSector(sector)}><Pencil size={16}/></button>}</li>)}</ul>}
        </div>
      </section>
    ...
    {draft && <Dialog title={draft.id ? 'Editar setor' : 'Novo setor'} onClose={() => !store.saving && setDraft(null)}><form onSubmit={saveSector} className="space-y-5"><label className="nv-label">Nome do setor<input className="nv-input" autoFocus maxLength={100} required placeholder="Ex.: Cirurgia Geral — Pronto-socorro" value={draft.name} onChange={event => setDraft({ ...draft, name: event.target.value })}/><span className="mt-1.5 block text-xs font-normal text-text-muted">Crie o setor uma vez e defina abaixo se ele funciona de dia, à noite ou nos dois períodos.</span></label><fieldset><legend className="nv-label">Cobertura do setor</legend><div className="grid gap-3 sm:grid-cols-2">{(['day','night'] as const).map(kind => { const period = draft.coveragePeriods.find(item => item.kind === kind); const Icon = kind === 'day' ? Sun : Moon; return <div key={kind} className={`rounded-xl border p-3 transition ${period ? 'border-primary/45 bg-primary/5' : 'border-border'}`}><label className="flex cursor-pointer items-center gap-3"><input type="checkbox" checked={Boolean(period)} onChange={() => toggleCoverage(kind)} className="h-4 w-4 rounded border-border text-primary focus:ring-primary"/><Icon size={17} className={kind === 'day' ? 'text-warning' : 'text-primary'}/><span><strong className="block text-sm text-text-primary">{coveragePeriodLabel(kind)}</strong><span className="text-xs text-text-muted">{kind === 'day' ? 'Ex.: 07:00 às 19:00' : 'Ex.: 19:00 às 07:00'}</span></span></label>{period && <div className="mt-3 grid grid-cols-2 gap-3 border-t border-border pt-3"><label className="nv-label">Início<input className="nv-input" type="time" required value={period.startTime} onChange={event => updateCoverage(kind, { startTime: event.target.value })}/></label><label className="nv-label">Fim<input className="nv-input" type="time" required value={period.endTime} onChange={event => updateCoverage(kind, { endTime: event.target.value })}/></label><label className="nv-label col-span-2">Médicos esperados<input className="nv-input" type="number" min={1} max={99} required value={period.requiredDoctors} onChange={event => updateCoverage(kind, { requiredDoctors: Math.max(1, Number(event.target.value)) })}/></label></div>}</div>; })}</div></fieldset><label className="nv-label">Situação<select className="nv-input" value={draft.status} onChange={event => setDraft({ ...draft, status: event.target.value as Sector['status'] })}><option value="active">Ativo</option><option value="inactive">Inativo</option></select></label><div><label className="nv-label">Especialidades aceitas<div className="flex gap-2"><input className="nv-input" list="specialty-options" placeholder="Ex.: Medicina intensiva" value={specialty} onChange={event => setSpecialty(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); addSpecialty(); } }}/><button type="button" className="nv-button-secondary" onClick={addSpecialty}>Adicionar</button></div></label><datalist id="specialty-options">{specialtyOptions.map(item => <option key={item} value={item}/>)}</datalist><div className="mt-2 flex flex-wrap gap-2">{draft.specialties.map(item => <button type="button" key={item} className="rounded-md bg-surface-muted px-2.5 py-1 text-xs" onClick={() => setDraft({ ...draft, specialties: draft.specialties.filter(value => value !== item) })}>{item} ×</button>)}</div></div>{formError && <p role="alert" className="rounded-lg border border-danger/30 bg-danger/5 px-3 py-2 text-sm text-danger">{formError}</p>}<div className="flex justify-end gap-3 pt-2"><button type="button" className="nv-button-secondary" onClick={() => setDraft(null)}>Cancelar</button><button className="nv-button" disabled={store.saving}>{store.saving ? 'Salvando…' : 'Salvar setor'}</button></div></form></Dialog>}
  </div></AccessGuard>;
}
