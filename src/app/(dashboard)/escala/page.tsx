...
import { AlertTriangle, ChevronLeft, ChevronRight, Clock3, Copy, List, LoaderCircle, Moon, Pencil, Plus, Repeat2, Sun, Users } from 'lucide-react';
import AccessGuard from '@/components/AccessGuard';
import Dialog from '@/components/Dialog';
import ScheduleCalendarView from '@/components/schedule/ScheduleCalendarView';
import { buildRecurringDates, coveragePeriodLabel, employmentLabels, hasConflict, localDate, paymentFrequencyLabels, sectorCoveragePeriods, summarizeSectorCoverage, type ShiftRecurrence } from '@/lib/scheduling';
  ...
  return <div className="overflow-hidden rounded-xl border border-border bg-card-bg">
    ...
    {days.map(day => { const dayItems = shifts.filter(item => item.date === day); const assigned = dayItems.filter(item => item.doctorId).length; const vacancies = dayItems.length - assigned; return <section key={day} className="border-b border-border last:border-0">
      ...
      <div className="divide-y divide-border">{dayItems.map(item => { const unit = units.find(value => value.id === item.unitId); const sector = sectors.find(value => value.id === item.sectorId); const doctor = doctors.find(value => value.id === item.doctorId); return <button key={item.id} type="button" onClick={() => onEdit(item)} className="grid w-full gap-2 px-4 py-3 text-left transition hover:bg-state-hover md:grid-cols-[110px_minmax(190px,1fr)_minmax(180px,1fr)_110px_120px_40px] md:items-center md:gap-4">
        ...
        <span className={`flex min-w-0 items-center gap-1.5 truncate text-sm font-medium ${doctor ? 'text-text-primary' : 'text-danger'}`}>{doctor?.name || 'Vaga aberta'}{doctor && doctorWarnings.get(doctor.id)?.warning && <AlertTriangle size={13} className={doctorWarnings.get(doctor.id)?.blocked ? 'shrink-0 text-danger' : 'shrink-0 text-warning'} aria-label="Pendência documental"/>}</span>
