...
import { Bell, ShieldAlert, Sparkles, Building, Sun, Moon } from 'lucide-react';
import Link from 'next/link';

export default function Header() {
  const { activeOrganizationId, organizations, documents, theme, setTheme } = useStore();
  ...
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-border bg-header-bg/85 backdrop-blur-md px-6 md:px-8 shadow-soft">
      ...
      <div className="flex items-center gap-3">
        <div className="md:hidden flex items-center gap-1.5 bg-primary/10 text-primary px-2 py-0.5 rounded text-xs font-semibold">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="p-1.5 rounded-full text-text-secondary hover:text-text-primary hover:bg-surface-muted transition cursor-pointer"
