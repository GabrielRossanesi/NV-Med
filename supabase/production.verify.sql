-- Run after 06_operational_scheduling.sql inside a transaction; always rollback fixtures.
...
DO $$ BEGIN
 ...
 INSERT INTO public.shifts(id,organization_id,doctor_id,unit_id,sector_id,date,start_time,end_time,type,status,specialty,employment_type,employer_name) VALUES('nv-test-shift','nv-test-a','nv-test-doctor','nv-test-unit','nv-test-sector','2099-01-01','19:00','07:00','onsite','confirmed','Clínico Geral','pj','Test company');
...
END $$;
RESET ROLE;
SELECT 'PASS: tenant isolation, sectors, open positions, anonymous blocking, write permissions, profile protection, required documents, night overlap, adjacent shift and deletion' AS verification;
ROLLBACK;
